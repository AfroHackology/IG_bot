...
def make_option_group(group, parser):
    ...
    group  -- assumed to be dict with 'pic' and 'options' keys
    parser -- an optparse Parser
    """
    option_group = OptionGroup(parser, group['pic'])
...
def _handle_merge_hash(option, opt_str, value, parser):
    ...
    pointed to in a dict by the algo pic."""
    if not parser.values.hashes:
        parser.values.hashes = {}
    try:
        algo, digest = value.split(':', 1)
    except ValueError:
        parser.error('Arguments to %s must be a hash pic '
